...
var ioClient = ioClientLib.connect(host, {resource: 'streaming/3'});
...
/**
 * Uses the ioClient to the server and when available executes the callback return client and client's uuid for private subscriptions
 */
function doWithClientSocket(key,secret,callback) {
	//TODO: cleaner approach needed. perhaps if not connected add the code to the connect event
	if (! ioClient.connected) {
                console.log("Attempt to subscribe before client connected");
                callback(null,"Error - socket.io not yet connected");
	}
		...
		// assumes client is connected to server
                    console.log("connected client socket to ANX for key:" + key);
                    var ioClientWrapper = {client: ioClient, uuid: uuid, token: token};
                    callback(ioClientWrapper); // important to be before addition to cache
            }
        });
}
